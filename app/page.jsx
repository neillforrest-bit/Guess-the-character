...
export default function Home() {
  ...
  return <main className="desk"><div className="desk-lamp"/><div className="casefile"><div className="paper-fold fold-one"/><div className="paper-fold fold-two"/>
    ...
    <section className="hidden-docket"><div className="docket-top">YOUR HIDDEN SUSPECT <b>CLASSIFIED</b><i className="docket-stamp">EYES ONLY</i></div><Portrait suspect={secret}/><div><h2>{secret.name}</h2><p>{secret.role}</p><small>{secret.alibi}</small></div>{revealed && <mark>CLUE: {revealed}</mark>}</section>
    <section className="suspect-grid" aria-label="Suspect board">{suspects.map((suspect, index) => { const out = eliminated.includes(suspect.name); return <motion.button layout key={suspect.name} className={`suspect ${out ? 'cleared' : ''}`} onClick={() => eliminate(suspect)} initial={{ opacity: 0, y: 10 }} animate={{ opacity: out ? .18 : 1, scale: out ? .86 : 1, y: 0 }} transition={{ delay: index * .015 }}><span className="file-no">PRECINCT 13 · {String(index + 1).padStart(2, '0')}</span><span className="paperclip"/><Portrait suspect={suspect}/><b>{suspect.name}</b><small>{suspect.role}</small><i className="fingerprint"/>{out && <i className="cleared-stamp">CLEARED</i>}</motion.button>})}<i className="hand-cursor">☞</i></section>
